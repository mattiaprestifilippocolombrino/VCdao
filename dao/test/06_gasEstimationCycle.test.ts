...
describe("Gas Estimation — Full Governance Cycle & Checkpoints", function () {
    ...
    it("4. Mint Tokens (Mint Stake + Warm SSTORE)", async function () {
        ...
        // Aggiungiamo stake e vediamo la differenza di costo rispetto al JoinDAO
    ...
    it("5. Upgrade Competences (VC Overhead vs Legacy)", async function () {
        ...
        gasReport["UpgradeCompetence VC (EIP-712)"] = receiptVP!.gasUsed;
        ...
        const txLeg = await token.connect(signerTL).upgradeCompetence(member2.address, 3, "PhDCS");
        const receiptLeg = await txLeg.wait();
        await network.provider.request({ method: "hardhat_stopImpersonatingAccount", params: [tlAddr] });
        
        gasReport["UpgradeCompetence Legacy (Direct)"] = receiptLeg!.gasUsed;
        const legacyCalldataCost = calculateCalldataCost(txLeg.data);

        gasReport["[Overhead EIP-712 (Crypto)]"] = gasReport["UpgradeCompetence VC (EIP-712)"] - gasReport["UpgradeCompetence Legacy (Direct)"];
    ...
    it("6. Proposta e Vote Lifecycle", async function () {
        // Creazione Proposta (il target diventa Professor = 4)
        const calldata = token.interface.encodeFunctionData("upgradeCompetence", [member1.address, 4, "Prof"]);
        const desc = "Promuovi Member1 a Professor";
    ...
    it("7. Complessità O(log N) nei Checkpoints", async function () {
        ...
        // Generiamo 10 checkpoint aumentando lo stake 10 volte (ogni transfer crea un checkpoint se minato in blocchi diversi)
        for (let i = 0; i < 10; i++) {
    ...
    it("8. Queue ed Execute Proposal", async function () {
        ...
        const calldata = token.interface.encodeFunctionData("upgradeCompetence", [member1.address, 4, "Prof"]);
        const descHash = ethers.id("Promuovi Member1 a Professor");
    ...
    it("Report Finale Formattato", async function () {
        ...
        for (const [action, gas] of Object.entries(gasReport)) {
            ...
            console.log(`${prefix}${action.padEnd(45)} | Gas: ${String(gas).padStart(8)} | USD: ${fmtUsd(gasCostInUsd(gas)).padStart(8)}`);
        ...
        console.log(`   ==================================================================================================`);
        console.log(`   - TOTALE GAS SPESO (ESCLUSO OVERHEAD/MISC)      | Gas: ${String(totalGas).padStart(8)} | USD: ${fmtUsd(gasCostInUsd(totalGas)).padStart(8)}`);
        console.log(`   ==================================================================================================\n`);
        ...
        console.log(`      Questo dimostra matematicamente che l'algoritmo di ricerca binaria scala in modo logaritmico, mantenendo`);
        console.log(`      i costi di scansione bassissimi rispetto al costo complessivo della transazione.`);
        console.log(`   ==================================================================================================\n`);
    });
});
