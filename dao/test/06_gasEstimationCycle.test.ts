...
describe("Gas Estimation — Full Governance Cycle & Checkpoints", function () {
    ...
    it("4. Increase Stake (Mint Stake + Warm SSTORE)", async function () {
        ...
        const tx = await token.connect(member1).increaseStake({ value: ethers.parseEther("1") });
        const receipt = await tx.wait();
        gasReport["Increase Stake (SSTORE Warm)"] = receipt!.gasUsed;
    });
    ...
    it("7. Complessità O(log N) nei Checkpoints", async function () {
        ...
        for (let i = 0; i < 10; i++) {
            await token.connect(member2).increaseStake({ value: ethers.parseEther("0.1") });
